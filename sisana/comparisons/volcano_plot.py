...
def plot_volcano(datafile: str, fcthreshold: str, adjpvalthreshold: str, numlabels: str, outdir: str):
    """
    Description:
    ...
    Parameters:
    -----------
        - datafile: str, Path to file containing the fold change, p-value, FDR, and mean 
    ...
    exp = pd.read_csv(datafile, index_col = 0)
    ...
    down = exp[(exp['logFC'] <= -1 * fcthreshold) & (exp['adjpval'] <= adjpvalthreshold)]
    up = exp[(exp['logFC'] >= fcthreshold) & (exp['adjpval'] <= adjpvalthreshold)]
    not_down_or_up = exp[(exp['logFC'] < fcthreshold) & (exp['logFC'] > -1 * fcthreshold) & (exp['adjpval'] < adjpvalthreshold)]
    notsig = exp[exp['adjpval'] > adjpvalthreshold]
    ...
    uptexts=[]
    if len(up) >= numlabels:
        up_subset = up.sort_values('adjpval', ascending=True).head(numlabels)
    ...
    if len(down) >= numlabels:
        dn_subset = down.sort_values('adjpval', ascending=True).head(numlabels)
